...
import { ALL_CONSUMABLES, CHARTS, CONTRACTS, RUMORS } from '../game/consumables.js';
...
export function compendiumScreen(game, back, tab = 'candles', query = '') {
  ...
  if (tab === 'candles') {
    ...
    const mk = (obj, label, cls) => {
      const list = Object.values(obj).filter((d) => hit(d.name, d.text));
      if (!list.length) return '';
      return `<h3>${label} · ${list.length}</h3><div class="comp-list">${list.map((d) =>
        `<div class="cl ${cls}"><span class="cl-art">${d.art}</span>
          <span class="cl-body"><b>${d.name}</b><small>${d.text}</small></span>
